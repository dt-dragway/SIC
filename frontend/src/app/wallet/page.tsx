...
export default function WalletPage() {
    ...
    const handleResetWallet = async () => {
        ...
        try {
            ...
            const res = await fetch('/api/v1/practice/reset', {
                method: 'POST',
            ...
            if (res.ok) {
                ...
                alert('Error al resetear el balance.');
            }
        } catch (error) {
                    ...
                    <div className="md:col-span-2 glass-card p-8 rounded-3xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="text-5xl font-bold text-white font-mono tracking-tight mb-6" suppressHydrationWarning>
                                {hideBalance ? '••••••' : (totalUsd > 0 && totalUsd < 1
                                    ? `$${(totalUsd ?? 0).toLocaleString('en-US', { minimumFractionDigits: 5, maximumFractionDigits: 5 })}`
                            ...
                            <div className="flex gap-4">
                                ...
                                {mode === 'practice' && (
                                    <button 
                                        ...
                                        className="px-4 py-2 rounded-xl border border-rose-500/30 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 font-medium text-xs transition-all flex items-center gap-2"
                                    >
                                ...
                                {balances.map((asset) => (
                                    <tr key={asset.asset} className="hover:bg-white/[0.02] transition-colors group">
                                        ...
                                        <td className="py-4 px-6 text-right font-mono font-medium text-emerald-400">
                                             {hideBalance ? '••••' : (asset.usd_value > 0 && asset.usd_value < 1
                                                 ? `$${asset.usd_value?.toLocaleString('en-US', { minimumFractionDigits: 5, maximumFractionDigits: 5 })}`
