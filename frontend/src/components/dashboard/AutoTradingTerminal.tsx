    ...
    return (
        <div className="bg-black/80 rounded-xl border border-white/10 overflow-hidden flex flex-col h-[500px] shadow-2xl mt-8">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar font-mono text-xs">
                {loading && terminalLogs.length === 0 && scanLogs.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-slate-500 gap-2">
                        <Activity size={24} className="animate-spin" />
                        <p>Iniciando interfaz de terminal...</p>
                    </div>
                ) : (terminalLogs.length === 0 && scanLogs.length === 0) ? (
                    ...
                    <div className="space-y-4">
                        ...
                        {/* Live Scanning Logs (Only show if running) */}
                        {isRunning && scanLogs.length > 0 && (
                            <div className="pt-4 border-t border-white/5 space-y-3">
                                <div className="text-[10px] text-slate-500 uppercase tracking-wider font-bold mb-2 flex items-center gap-1.5 font-sans">
                                    <span className="flex h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse"></span>
                                    Monitoreo de Escaneo Live
                                </div>
                                {[...scanLogs].reverse().slice(0, 10).map((log) => {
                                    const timeStr = new Date(log.timestamp).toLocaleTimeString();
                                    const isApproved = log.message.includes('APROBADA');
                                    const isRejection = log.message.includes('descartada');
                                    const isError = log.message.includes('Error');
                                    let colorClass = 'text-slate-400';
                                    if (isApproved) colorClass = 'text-emerald-400 font-bold';
                                    else if (isRejection) colorClass = 'text-amber-400/80';
                                    else if (isError) colorClass = 'text-rose-400';

                                    return (
                                        <div key={log.id} className="flex gap-2.5 items-start pl-1 text-[11px] leading-relaxed">
                                            <span className="text-slate-600 shrink-0">[{timeStr}]</span>
                                            <span className="text-blue-500 font-semibold shrink-0">[{log.symbol}]</span>
                                            <span className={`${colorClass} opacity-90`}>{log.message}</span>
                                        </div>
                                    );
                                })}
                            </div>
