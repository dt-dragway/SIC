        ...
        <div className="bg-black/40 rounded-3xl border border-white/5 overflow-hidden flex flex-col h-full shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar font-mono text-[11px]">
                {loading && logs.length === 0 ? (
                    ...
                    logs.map((log, i) => (
                        <div key={log.id} className={`border-l-2 ${log.side === 'BUY' ? 'border-emerald-500/30' : log.side === 'SELL' ? 'border-rose-500/30' : 'border-blue-500/30'} pl-3 py-1 animate-in slide-in-from-left-2 duration-300`} style={{ animationDelay: `${i * 50}ms` }}>
                            ...
                            <div className="mt-1 text-slate-500 text-[10px] flex gap-3 font-mono">
                                <span>SALDO: ${log.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USDT</span>
                                <span>RADAR IMBALANCE: {log.quantity.toFixed(2)}x</span>
                            </div>
