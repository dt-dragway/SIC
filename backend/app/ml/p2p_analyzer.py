...
class P2POpportunityFinder:
    ...
    async def analyze_offer_context(self, best_offer: Dict, market_data: Dict, criteria: Optional[Dict] = None) -> str:
        """
        Genera un razonamiento avanzado usando LLM para la mejor oferta considerando criterios.
        """
        profile_desc = "⚖️ BALANCEADO (Equilibrio de precio y seguridad)"
        if criteria:
            pw = criteria.get('price_weight', 0.5)
            sw = criteria.get('safety_weight', 0.5)
            if pw >= 0.7:
                profile_desc = "⚡ MÁXIMO RENDIMIENTO (Foco prioritario en mejor precio neto)"
            elif sw >= 0.7:
                profile_desc = "🛡️ SEGURIDAD MÁXIMA (Foco prioritario en reputación del comerciante)"
        
        if not self.llm_manager.is_available:
             return f"La IA seleccionó a {best_offer.get('advertiser', 'N/A')} bajo perfil {profile_desc} por su precio competitivo y tasa de finalización ({best_offer.get('completion_rate', 0):.1f}%)."

        prompt = f"""
        ANÁLISIS DE OPORTUNIDAD P2P (Arbitraje/Trading)
        
        Perfil de Criterio IA Solicitado: {profile_desc}
        ...
        Genera una razón corta (max 1 frase potente) y persuasiva de por qué esta es la mejor opción ahora mismo bajo el perfil {profile_desc}.
        Usa emojis. Enfócate en la seguridad, rapidez o beneficio económico de acuerdo al perfil seleccionado.
        """
        
        response = await self.llm_manager._active_provider.analyze(prompt)
        return response if response else f"Oportunidad verificada por algoritmos de alta frecuencia bajo perfil {profile_desc}."
