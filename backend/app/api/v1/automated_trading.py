...
async def get_automation_status(
    ...
    try:
        ...
        # Sincronizar estado en vivo con la Base de Datos para evitar el "efecto espejismo"
        from app.infrastructure.database.models import AutomationConfig
        config = db.query(AutomationConfig).filter(AutomationConfig.user_id == current_user.id).first()
        
        # Si en la DB está marcado como enabled pero en RAM no está corriendo, 
        # intentamos restaurarlo silenciosamente para corregir el estado.
        if config and config.enabled and not auto_service.running:
            logger.info(f"🔄 Restaurando estado 'running' para usuario {current_user.id} detectado desincronizado.")
            settings_dict = {
            ...
            import asyncio
            asyncio.create_task(auto_service.start_automation(int(current_user.id), settings_dict))
            status['running'] = True
        elif config:
