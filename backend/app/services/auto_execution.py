...
class AutoExecutionService:
    ...
    async def _update_practice_wallet_balance(self, user_id: int, symbol: str, side: str, quantity: float, price: float, is_futures: bool = False) -> bool:
        """
        Actualiza el saldo de la wallet de práctica ante cada compra/venta del bot IA.
        Soporta SPOT (requiere activos para vender) y FUTUROS (usa margen USDT).
        """
        db = SessionLocal()
        try:
            from app.api.v1.practice import get_or_create_wallet
            from app.infrastructure.database.models import VirtualTrade as VirtualTradeModel, VirtualPosition as VirtualPositionModel
            ...
            # --- Lógica de FUTUROS (Hedge/Short) ---
            if is_futures:
                # En futuros usamos margen (x5 por defecto para el bot si no se especifica)
                leverage = self.settings.get('leverage', 5)
                required_margin = total_notional / leverage
                fee = total_notional * 0.001
                
                usdt_balance = float(balances.get("USDT", 0))
                
                if usdt_balance < (required_margin + fee):
                    logger.warning(f"⚠️ [IA FUTUROS] Saldo insuficiente para margen: {required_margin + fee:.2f} USDT")
                    return False
                
                # Descontar margen y fee
                balances["USDT"] = round(usdt_balance - required_margin - fee, 2)
                wallet.balances = json_lib.dumps(balances)
                
                # Crear la posición de futuros en la DB
                # Calculamos un precio de liquidación aproximado
                ...
                new_pos = VirtualPositionModel(
                    wallet_id=wallet.id, symbol=symbol, side=side_label, size=quantity,
                    entry_price=price, leverage=leverage, margin=required_margin,
                    liquidation_price=round(liq_price, 4), created_at=datetime.utcnow()
                )
                db.add(new_pos)
                
                logger.info(f"📈 [IA FUTUROS] {side_label} abierto: {quantity} {symbol} @ ${price} (Margen: {required_margin:.2f} USDT)")
            ...
            # --- Lógica de SPOT ---
                ...
                if side.upper() == "BUY":
                    usdt_balance = float(balances.get("USDT", 0))
                    if usdt_balance >= total_notional + fee:
                        balances["USDT"] = round(usdt_balance - total_notional - fee, 2)
                        balances[base_asset] = round(float(balances.get(base_asset, 0)) + quantity, 8)
                        logger.info(f"💰 [IA SPOT] Compra: -{total_notional + fee:.2f} USDT. +{quantity} {base_asset}")
                    else:
                        logger.warning(f"⚠️ [IA SPOT] Saldo insuficiente en USDT.")
                        return False
                elif side.upper() == "SELL":
                    asset_balance = float(balances.get(base_asset, 0))
                    if asset_balance >= quantity:
                        balances[base_asset] = round(asset_balance - quantity, 8)
                        balances["USDT"] = round(float(balances.get("USDT", 0)) + total_notional - fee, 2)
                        if balances[base_asset] <= 0.00000001: del balances[base_asset]
                        logger.info(f"💰 [IA SPOT] Venta: -{quantity} {base_asset}. +{total_notional - fee:.2f} USDT")
                    else:
                        logger.warning(f"⚠️ [IA SPOT] No tienes {base_asset} para vender.")
                        return False
            
            # Guardar balances y registrar trade
            wallet.balances = json_lib.dumps(balances)
            new_trade = VirtualTradeModel(
                wallet_id=wallet.id, symbol=symbol, side=side.upper(), type="MARKET",
                strategy="AI_AUTO", reason="Entrenamiento: Ejecución automática",
                quantity=quantity, price=price, pnl=0.0,
            ...
            logger.error(f"❌ Error en balance de práctica: {e}")
            db.rollback()
            return False
        finally:
            db.close()
    ...
    def _check_daily_loss_limit(self) -> bool:
        """
        Verifica si se alcanzó el límite de pérdidas diario ($0.50 USD).
        Calcula el PnL realizado en las últimas 24 horas.
        """
        db = SessionLocal()
        try:
            from app.infrastructure.database.models import VirtualTrade
            
            # Límite estricto para cuenta de $20 (2.5% drawdown)
            DAILY_LOSS_LIMIT = -0.50 
            
            # Obtener trades de las últimas 24 horas para este usuario
            yesterday = datetime.utcnow() - timedelta(days=1)
            user_id = self.user_id if hasattr(self, 'user_id') else 1
            ...
            # Solo sumar PnL de cierres (SELL) que tengan PnL registrado
            trades_24h = db.query(VirtualTrade).filter(
                VirtualTrade.wallet_id == wallet.id,
                VirtualTrade.created_at >= yesterday,
                VirtualTrade.side == "SELL"
            ).all()
            
            total_pnl_24h = sum(float(t.pnl or 0) for t in trades_24h)
            
            if total_pnl_24h <= DAILY_LOSS_LIMIT:
                logger.warning(f"🚨 LÍMITE DE PÉRDIDA DIARIO ALCANZADO: ${total_pnl_24h:.2f} (Límite: ${DAILY_LOSS_LIMIT:.2f})")
                return True
                
            return False
        except Exception as e:
            logger.error(f"❌ Error verificando límite de pérdidas: {e}")
            return False
        finally:
            db.close()
    ...
    def _check_consecutive_losses(self) -> bool:
        """
        Verifica si hay demasiadas pérdidas consecutivas (Límite: 5).
        """
        db = SessionLocal()
        try:
            ...
            # Obtener los últimos 5 trades cerrados
            last_trades = db.query(VirtualTrade).filter(
                VirtualTrade.wallet_id == wallet.id,
                VirtualTrade.side == "SELL"
            ).order_by(VirtualTrade.created_at.desc()).limit(5).all()
            
            if len(last_trades) < 5:
                return False
                
            # Si todos los últimos 5 son negativos
            all_losses = all(float(t.pnl or 0) < 0 for t in last_trades)
            
            if all_losses:
                logger.warning(f"🚨 RACHA DE PÉRDIDAS CRÍTICA: 5 trades consecutivos en rojo. Deteniendo sistema.")
                return True
                
            return False
        except Exception as e:
            logger.error(f"❌ Error verificando racha de pérdidas: {e}")
            return False
        finally:
            db.close()
